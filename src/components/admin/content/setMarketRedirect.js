...
import {setmarket,EditMarket} from '../../../store/actions/setMarketAction';
Geocode.setApiKey( "AIzaSyCd5GSrdhkRjDu53HCBVL7fh5QXa1-gIBE" );
Geocode.enableDebug();

class SetMarketRedirect extends Component {
    state = {
        searched:'',
        MarketName:'',
        markerPosition: [],   
    ...
    onhandleSubmitform=e=>{
        ...
        {
            this.setState({
                formFill:'Please provide the name of the market'
            })
            return
        }
        if(this.state.north===''||this.state.south==='')
        {
            this.setState({
                formFill:'Please provide the location of the market'
            })
            return
        }
        else{
        ...
        if(this.props.id==='')
        {
            console.log(this.props.Market.filter(item=>item.MarketName===this.state.MarketName))
           if( this.props.Market.filter(item=>item.MarketName===this.state.MarketName).length!==0)
           {
            this.setState({
                formFill:'The Market Name already in use'
            }) 
            return  
           }
           this.setState({
        ...
        this.props.EditMarket(this.props.id,{MarketName:this.state.MarketName,northEast:{lat:this.state.north,lng:this.state.east},southWast:{lat:this.state.south,lng:this.state.west}})
        }
    ...
    render() {
        console.log(this.props.id,'the id is here')
        ...
        return (
            <div >
                ...
                {this.state.formFill!=='false'&&this.state.formFill===''?<p style={{fontSize:'20px',marginLeft:'28%',color:'#82ae46'}}>{this.props.marketSetError}</p>:null}
                <h4>Market Name:</h4>
                ...
                {this.state.formFill!=='false'&&this.state.formFill===''?<p style={{fontSize:'20px',marginLeft:'28%',color:'#82ae46'}}>{this.props.marketSetError}</p>:null}
                {this.state.formFill!=='false'?<p style={{fontSize:'18px',marginLeft:'28%',color:'red'}}>{this.state.formFill}</p>:null}
                </div>
            </div>
        );
    }
...
const mapStateToProps=(state)=>{
  ...
  const mapDispatchToProps=(dispatch)=>{
    return{
        ...
        EditMarket:(id,uploads)=>dispatch(EditMarket (id,uploads))
    }
  }
